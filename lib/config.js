...

{
    ...
    chrome.i18n.getAcceptLanguages(acceptedLanguages => {
        chrome.storage.local.get(null, onGot => {
            ...
            config.targetLanguages = config.targetLanguages.map(lang => fixTLanguageCode(lang))
            config.neverTranslateLangs = config.neverTranslateLangs.map(lang => fixTLanguageCode(lang))
            config.alwaysTranslateLangs = config.alwaysTranslateLangs.map(lang => fixTLanguageCode(lang))
            config.targetLanguage = fixTLanguageCode(config.targetLanguage)
            config.targetLanguageTextTranslation = fixTLanguageCode(config.targetLanguageTextTranslation)
